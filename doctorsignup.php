...
if(isset($_POST['submit'])){
   ...
   $role=$_POST['role'];
  $licno1=$_POST['lic_no'];
   $clinicname1=$_POST['clinic_name'];
   $hc_no= "i".substr($aadharno,6)."doc".substr($birthdate,8)."lth";
  ...
  $selectresults = mysqli_query($conn,$slsquery);
    ...
    elseif(mysqli_num_rows($selectresults)>0){
      ...
      $query = "INSERT INTO signup (`f_name`,`hc_no`,`role_user`,`l_name`,`aadhar_no`,`email`,`clinicname`,`licno`,`password`,`address`,`birth_date`,`city`,`phone_no`,`alt_phone_no`) VALUES ('$u_firstname','$hc_no','$role','$u_lastname','$aadharno','$email','$clinicname1','$licno1','$password','$address','$birthdate','$city','$phoneno','$altphone')";
...
<p style="color:red;"><?php if(isset($msg)){ echo $msg;} ?></p>
          <form class="text-left clearfix mt-30" method="POST">
            ...
            <div class="form-group">
              <input type="text" class="form-control"  placeholder="clinic name" name="clinic_name" required="true">
            </div>
            <div class="form-group">
              <input type="text" class="form-control"  placeholder="License no" name="lic_no" required="true">
            </div>
            <div class="form-group">
