...
if(isset($_POST['submit'])){
   ...
   $role=$_POST['role'];
   $labid=$_POST['lab_id'];
   $hc_no= "i".substr($aadharno,6)."lab".substr($birthdate,8)."lth";
  ...
  $selectresults = mysqli_query($conn,$slsquery);
    ...
    elseif(mysqli_num_rows($selectresults)>0){
      ...
      $query = "INSERT INTO signup (`f_name`,`hc_no`,`role_user`,`l_name`,`aadhar_no`,`email`,`labid`,`password`,`address`,`birth_date`,`city`,`phone_no`,`alt_phone_no`) VALUES ('$u_firstname','$hc_no','$role','$u_lastname','$aadharno','$email','$labid','$password','$address','$birthdate','$city','$phoneno','$altphone')";
...
<p style="color:red;"><?php if(isset($msg)){ echo $msg;} ?></p>
          <form class="text-left clearfix mt-30" method="POST">
            ...
            <input type="hidden" name="role" value="laboratory">
            <div class="form-group">
              <input type="text" class="form-control"  placeholder="Lab Id" name="lab_id">
            </div>
            <div class="form-group">
